...
int32_t init_memory()
{
    ...
    for (i = 0x28; i < MemoryMaxCount; i++) readrdramreg[i] = &trash;
    for (i = 1; i < 0x10; i++)
    {
    ...
    for (i = 0x20; i < MemoryMaxCount; i++) readrspreg[i] = &trash;
    for (i = 5; i < 8; i++)
    {
        readmem[0x8400 + i] = read_nothing;
        readmem[0xa400 + i] = read_nothing;
    ...
    for (i = 0x8; i < MemoryMaxCount; i++) readrsp[i] = &trash;
    for (i = 9; i < 0x10; i++)
    {
        readmem[0x8400 + i] = read_nothing;
        readmem[0xa400 + i] = read_nothing;
    ...
    for (i = 0x20; i < MemoryMaxCount; i++) readdp[i] = &trash;
    for (i = 1; i < 0x10; i++)
    {
    ...
    for (i = 0x10; i < MemoryMaxCount; i++) readdps[i] = &trash;
    for (i = 1; i < 0x10; i++)
    {
    ...
    for (i = 0x10; i < MemoryMaxCount; i++) readmi[i] = &trash;
    for (i = 1; i < 0x10; i++)
    {
    ...
    for (i = 0x38; i < MemoryMaxCount; i++) readvi[i] = &trash;
    for (i = 1; i < 0x10; i++)
    {
    ...
    for (i = 0x18; i < MemoryMaxCount; i++) readai[i] = &trash;
    for (i = 1; i < 0x10; i++)
    {
    ...
    for (i = 0x34; i < MemoryMaxCount; i++) readpi[i] = &trash;
    for (i = 1; i < 0x10; i++)
    {
    ...
    for (i = 0x20; i < MemoryMaxCount; i++) readri[i] = &trash;
    for (i = 1; i < 0x10; i++)
    {
    ...
    for (i = 0x1c; i < MemoryMaxCount; i++) readsi[i] = &trash;
...
static void update_MI_init_mode_reg()
{
    ...
    if (MI_register.w_mi_init_mode_reg & 0x80) MI_register.init_mode = 0;
    if (MI_register.w_mi_init_mode_reg & 0x100) MI_register.init_mode = 1;
    if (MI_register.w_mi_init_mode_reg & 0x200) MI_register.ebus_test_mode = 0;
    if (MI_register.w_mi_init_mode_reg & 0x400) MI_register.ebus_test_mode = 1;
    if (MI_register.w_mi_init_mode_reg & 0x800)
    {
        MI_register.mi_intr_reg &= 0xFFFFFFDF;
        check_interrupt();
    }
    if (MI_register.w_mi_init_mode_reg & 0x1000) MI_register.RDRAM_reg_mode = 0;
    if (MI_register.w_mi_init_mode_reg & 0x2000) MI_register.RDRAM_reg_mode = 1;
    MI_register.mi_init_mode_reg = ((MI_register.init_length) | (MI_register.init_mode << 7) |
                                    (MI_register.ebus_test_mode << 8) | (MI_register.RDRAM_reg_mode << 9));
}
...
void update_SP()
{
    if (sp_register.w_sp_status_reg & 0x1) sp_register.halt = 0;
    if (sp_register.w_sp_status_reg & 0x2) sp_register.halt = 1;
    if (sp_register.w_sp_status_reg & 0x4) sp_register.broke = 0;
    ...
    if (sp_register.w_sp_status_reg & 0x20) sp_register.single_step = 0;
    if (sp_register.w_sp_status_reg & 0x40) sp_register.single_step = 1;
    if (sp_register.w_sp_status_reg & 0x80) sp_register.intr_break = 0;
    if (sp_register.w_sp_status_reg & 0x100) sp_register.intr_break = 1;
    if (sp_register.w_sp_status_reg & 0x200) sp_register.signal0 = 0;
    if (sp_register.w_sp_status_reg & 0x400) sp_register.signal0 = 1;
    if (sp_register.w_sp_status_reg & 0x800) sp_register.signal1 = 0;
    if (sp_register.w_sp_status_reg & 0x1000) sp_register.signal1 = 1;
    if (sp_register.w_sp_status_reg & 0x2000) sp_register.signal2 = 0;
    if (sp_register.w_sp_status_reg & 0x4000) sp_register.signal2 = 1;
    if (sp_register.w_sp_status_reg & 0x8000) sp_register.signal3 = 0;
    if (sp_register.w_sp_status_reg & 0x10000) sp_register.signal3 = 1;
    if (sp_register.w_sp_status_reg & 0x20000) sp_register.signal4 = 0;
    if (sp_register.w_sp_status_reg & 0x40000) sp_register.signal4 = 1;
    if (sp_register.w_sp_status_reg & 0x80000) sp_register.signal5 = 0;
    if (sp_register.w_sp_status_reg & 0x100000) sp_register.signal5 = 1;
    if (sp_register.w_sp_status_reg & 0x200000) sp_register.signal6 = 0;
    if (sp_register.w_sp_status_reg & 0x400000) sp_register.signal6 = 1;
    if (sp_register.w_sp_status_reg & 0x800000) sp_register.signal7 = 0;
    ...
    if (!sp_register.halt && !sp_register.broke)
    {
        ...
        if (SP_DMEM[0xFC0 / 4] == 1)
        {
                ...
                frameBufferInfos[0].addr)
            {
                ...
                for (i = 0; i < 6; i++)
                {
                    if (frameBufferInfos[i].addr)
                    {
                        ...
                        int32_t end =
                            start + frameBufferInfos[i].width * frameBufferInfos[i].height * frameBufferInfos[i].size -
                            1;
                        start = start >> 16;
                        end = end >> 16;
                        for (j = start; j <= end; j++)
            ...
            if (g_core->video_fb_get_frame_buffer_info && g_core->video_fb_read && g_core->video_fb_write)
            {
                g_core->video_fb_get_frame_buffer_info(frameBufferInfos);
            }
                ...
                for (i = 0; i < 6; i++)
                {
                    if (frameBufferInfos[i].addr)
                    {
                        ...
                        if (firstFrameBufferSetting)
                        {
                            ...
                            fast_memory = 0;
                            for (j = 0; j < 0x100000; j++) invalid_code[j] = 1;
                        }
                    }
                }
            }
...
void update_DPC()
{
    if (dpc_register.w_dpc_status & 0x1) dpc_register.xbus_dmem_dma = 0;
    if (dpc_register.w_dpc_status & 0x2) dpc_register.xbus_dmem_dma = 1;
    if (dpc_register.w_dpc_status & 0x4) dpc_register.freeze = 0;
    if (dpc_register.w_dpc_status & 0x8) dpc_register.freeze = 1;
    if (dpc_register.w_dpc_status & 0x10) dpc_register.flush = 0;
...
void write_rdramFB()
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (frameBufferInfos[i].addr)
        {
            int32_t start = frameBufferInfos[i].addr & 0x7FFFFF;
            int32_t end = start + frameBufferInfos[i].width * frameBufferInfos[i].height * frameBufferInfos[i].size - 1;
            if ((address & 0x7FFFFF) >= start && (address & 0x7FFFFF) <= end) g_core->video_fb_write(address, 4);
        }
    }
    write_rdram();
}
...
void write_rdramFBb()
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (frameBufferInfos[i].addr)
        {
            int32_t start = frameBufferInfos[i].addr & 0x7FFFFF;
            int32_t end = start + frameBufferInfos[i].width * frameBufferInfos[i].height * frameBufferInfos[i].size - 1;
            if ((address & 0x7FFFFF) >= start && (address & 0x7FFFFF) <= end) g_core->video_fb_write(address ^ S8, 1);
        }
    }
    write_rdramb();
}
...
void write_rdramFBh()
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (frameBufferInfos[i].addr)
        {
            int32_t start = frameBufferInfos[i].addr & 0x7FFFFF;
            int32_t end = start + frameBufferInfos[i].width * frameBufferInfos[i].height * frameBufferInfos[i].size - 1;
            if ((address & 0x7FFFFF) >= start && (address & 0x7FFFFF) <= end) g_core->video_fb_write(address ^ S16, 2);
        }
    }
    write_rdramh();
}
...
void write_rdramFBd()
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (frameBufferInfos[i].addr)
        {
            int32_t start = frameBufferInfos[i].addr & 0x7FFFFF;
            int32_t end = start + frameBufferInfos[i].width * frameBufferInfos[i].height * frameBufferInfos[i].size - 1;
            if ((address & 0x7FFFFF) >= start && (address & 0x7FFFFF) <= end) g_core->video_fb_write(address, 8);
        }
    }
    write_rdramd();
}
...
void read_rdramregd()
{
    *rdword = ((uint64_t)(*readrdramreg[*address_low]) << 32) | *readrdramreg[*address_low + 4];
}
...
void read_rsp_regd()
{
    *rdword = ((uint64_t)(*readrspreg[*address_low]) << 32) | *readrspreg[*address_low + 4];
    switch (*address_low)
    {
    case 0x18:
        sp_register.sp_semaphore_reg = 1;
...
void read_rspd()
{
    *rdword = ((uint64_t)(*readrsp[*address_low]) << 32) | *readrsp[*address_low + 4];
}
...
void read_dpd()
{
    *rdword = ((uint64_t)(*readdp[*address_low]) << 32) | *readdp[*address_low + 4];
}
...
void read_dpsd()
{
    *rdword = ((uint64_t)(*readdps[*address_low]) << 32) | *readdps[*address_low + 4];
}
...
void read_mid()
{
    *rdword = ((uint64_t)(*readmi[*address_low]) << 32) | *readmi[*address_low + 4];
}
...
void read_vid()
{
    ...
    *rdword = ((uint64_t)(*readvi[*address_low]) << 32) | *readvi[*address_low + 4];
}
...
void write_vi()
{
    ...
    case 0x0:
        if (vi_register.vi_status != word)
        {
            ...
            g_core->video_vi_status_changed();
        }
        return;
        break;
    case 0x8:
...
void read_ai()
{
    ...
    case 0x4:
        ...
        if (ai_register.current_delay != 0 && get_event(AI_INT) != 0 && (get_event(AI_INT) - core_Count) < 0x80000000)
            *rdword = ((get_event(AI_INT) - core_Count) * (int64_t)ai_register.current_len) / ai_register.current_delay;
        else
            *rdword = 0;
        return;
        break;
...
void read_aib()
{
    ...
    case 0x7:
        ...
        if (ai_register.current_delay != 0 && get_event(AI_INT) != 0)
            len = ((get_event(AI_INT) - core_Count) * (int64_t)ai_register.current_len) / ai_register.current_delay;
        else
            len = 0;
...
void read_aid()
{
    ...
    *rdword = ((uint64_t)(*readai[*address_low]) << 32) | *readai[*address_low + 4];
}
...
void write_ai()
{
    ...
    case 0x4:
        ...
        case 0x59: {
            ...
            if (f) delay = ((uint64_t)ai_register.ai_len * vi_register.vi_delay * 50) / (f * 4);
        }
        break;
        case 0x37:
        case 0x41:
        ...
        case 0x4a: {
            ...
            if (f) delay = ((uint64_t)ai_register.ai_len * vi_register.vi_delay * 60) / (f * 4);
        }
        break;
        }
    ...
    case 0x10:
        if (ai_register.ai_dacrate != word)
        {
            ...
            g_core->audio_ai_dacrate_changed(g_sys_type);
...
void write_aib()
{
    ...
    case 0x7:
        ...
        *((unsigned char *)&temp + ((*address_low & 3) ^ S8)) = g_byte;
        ...
        case 0x59:
            delay =
                ((uint64_t)ai_register.ai_len * (ai_register.ai_dacrate + 1) * vi_register.vi_delay * 50) / 49656530;
            break;
        case 0x37:
        case 0x41:
        case 0x45:
        case 0x4a:
            delay =
                ((uint64_t)ai_register.ai_len * (ai_register.ai_dacrate + 1) * vi_register.vi_delay * 60) / 48681812;
            break;
        }
        // delay = 0;
        if (ai_register.ai_status & 0x40000000) // busy
...
void read_pid()
{
    *rdword = ((uint64_t)(*readpi[*address_low]) << 32) | *readpi[*address_low + 4];
}
...
void read_rid()
{
    *rdword = ((uint64_t)(*readri[*address_low]) << 32) | *readri[*address_low + 4];
}
...
void read_sid()
{
    *rdword = ((uint64_t)(*readsi[*address_low]) << 32) | *readsi[*address_low + 4];
}
